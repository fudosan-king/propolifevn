...
function headerSeo($seo = array(), $seoGoogle = array())
{
    ...
    if(!isset($seo['image'])){
        $meta_string .= '<meta property="og:image" content="'. PATH_URL . "static/images/default.jpg".'" />';
    }
...
function sendEmail($emailTo, $emailToName, $fromEmail, $fromName, $subject, $body, $bcc_list = array())
{
    $CI =& get_instance();
    ...
    $mail->SMTPAuth   = true; 
    $mail->SMTPSecure = "tls"; 
    $mail->Host       = "mail.aozaihousing.sakura.ne.jp";   
    $mail->Port       = 587;   
    $mail->Username   = "tandat@aozaihousing.sakura.ne.jp";  
    $mail->Password   = "tandat86";
    $mail->CharSet = "utf-8";
...
function cutStringJP($str,$number){
    mb_internal_encoding("UTF-8");
    $data=mb_substr($str,0,$number);
    return $data;
}
?>