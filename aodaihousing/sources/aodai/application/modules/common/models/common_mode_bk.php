...
class Common_model extends MY_Model {
    ...
    function search_model($data, $per_page, $cur_page){
        ...
        if($data['s'] != '') {
            $this->db->like('name_jp', $data['s']);
            //$this->db->or_like(PREFIX.'areas.name', $data['s']);
            
            $data_sql = array();
            
            
            //$this->db->or_like(PREFIX.$data['table'].'.introduction', $data['s']);
            //$this->db->or_like(PREFIX.$data['table'].'.comment', $data['s']);
            //$this->db->or_like(PREFIX.$data['table'].'.other_notice', $data['s']);
            $data_sql[] = PREFIX.'areas.name' . ' like %' .$data['s'] . '%';
            $data_sql[] = PREFIX.$data['table'].'.introduction' . ' like %' .$data['s'] . '%';
            $data_sql[] = PREFIX.$data['table'].'.comment' . ' like %' . $data['s'] . '%';
            
            if($data['table'] == 'houses'){
                if(is_numeric($data['s'])) {
                    //$this->db->or_where(PREFIX.$data['table'].'.size', $data['s']);
                    //$this->db->or_where(PREFIX.$data['table'].'.rent', $data['s']);
                    
                    $data_sql[] = PREFIX.$data['table'].'.size' . ' like %' . $data['s'] . '%';
                    $data_sql[] = PREFIX.$data['table'].'.rent' . ' like %' . $data['s'] . '%';
                }
                if(is_string($data['s'])){
                    //$this->db->or_like(PREFIX.$data['table'].'.house_layout_id', $data['s']);
                    $data_sql[] = PREFIX.$data['table'].'.house_layout_id' . ' like %' . $data['s'] . '%';
                }
                
            }
            
            if($data['table'] == 'office'){
                if(is_numeric($data['s'])) {
                    //$this->db->or_where(PREFIX.$data['table'].'.size', $data['s']);
                    $data_sql[] = PREFIX.$data['table'].'.size' . ' like %' . $data['s'] . '%';
                }
                if(is_string($data['s'])){
                    //$this->db->or_like(PREFIX.$data['table'].'.month_rent', $data['s']);
                    //$this->db->or_like(PREFIX.$data['table'].'.size_rent', $data['s']);
                    $data_sql[] = PREFIX.$data['table'].'.month_rent' . ' like %' . $data['s'] . '%';
                    $data_sql[] = PREFIX.$data['table'].'.size_rent' . ' like %' . $data['s'] . '%';
                }
            }
