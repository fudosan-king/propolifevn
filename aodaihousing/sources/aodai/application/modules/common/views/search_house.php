...
// echo $catname;die;
    ...
    $text_type = array(
        ...
        1 =>lang('ホーチミン コンドミニアム'),
        2 => lang('ホーチミン 戸建・店舗')
    );
    if($type != ''){
        $catname = $data_type[$type];
    }
    if(!empty($localtion)){
        $catname = lang($itemAreas[$localtion]);
    }
        
    if($type != '' && !empty($localtion) && (empty($rent) && empty($layout) && empty($size))){
        $catname = lang($itemAreas[$localtion]).'  '.$data_type[$type];
    }
    if($type != '' && !empty($localtion) && (!empty($rent) || !empty($layout) || !empty($size))){
        $catname = lang($itemAreas[$localtion]).'  '.$data_type[$type];
    }
    ...
    <?php if(!empty($items)): ?>
        ...
        <div class="deatil-top <?=$lastChild?>">
            ...
            <p class="read-more"><a href="<?=create_url("houses/detail/" . $item->id . '/' . vcp_printf($item->name_en, current_lang_()));?>">
                <img src="<?=PATH_URL_IMAGES?>static/images/read-more<?=current_lang();?>.jpg" /></a>
            </p>
        </div>
        <?php endforeach; ?>
