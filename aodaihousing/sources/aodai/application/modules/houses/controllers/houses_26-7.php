...
//        getToolbar($this->module, $toolBar);
    ...
    public function detail() {
        ...
        if (is_numeric($id) == false) {
            ...
            $size = !empty($item->size) ? vcp_printf($item->size, current_lang_()) . ' m&#178;' : '';
            $rent = !empty($item->rent) ? vcp_printf($item->rent, current_lang_()) . ' USD' : '';
            $title = '【物件情報】' . vcp_printf($item->area, current_lang_()) . ' - ' . $rent . ' - ' . $size . ' | アオザイハウジング';
            $description = empty($item->introduction) ? $this->config->item('description') : vcp_printf($item->introduction, current_lang_());
            $thumb = empty($item->images_thumb) ? $this->config->item('image') : PATH_URL . 'static/uploads/houses/' . $item->images_thumb;
            
            $seoGoogle = array(
                'keyword' => 'ホーチミン,サービスアパー ト,アパート,戸建'
            );
            $seo = array(
                'image' => $thumb,
                'title' => $title,
    ...
    public function home_list_ofices() {
        $data = array(
            'items' => $this->model->getListItemOffice(6, 0)
        );
