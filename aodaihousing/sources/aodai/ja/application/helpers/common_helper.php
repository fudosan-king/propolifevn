...
/**
 * Use for debug display and exit data
 */
...
function autoGenerateFolder($path_folder) {
	//set path of folder where to save file
...
if ( ! (function_exists('helper_delete_file'))) {
	function helper_delete_file($path_folder, $file_name) {
		@unlink(BASEFOLDER . $path_folder . '/' . $file_name);
	}
}
...
function getImage($src,$maxw=0,$maxh=0){
	...
	if($maxw!=0){
		if($maxw>$w){
			return '<img alt="" src="'.PATH_URL.$src.'" />';
		}else{
			return '<img alt="" width="'.$maxw.'" src="'.PATH_URL.$src.'" />';
		}
	}
	...
	if($maxh!=0){
		if($maxh>$h){
			...
			return '<img alt="" width="'.$maxh.'" src="'.PATH_URL.$src.'" />';
		}
	}
}
...
function headerSeo($seo = array(), $seoGoogle = array())
{
    ...
    foreach($meta as $item) {
        ...
        if($item->key == 'name="keywords"' && $seoGoogle && isset($seoGoogle['keyword'])){
...
function getToolbar($module, $items = array()) {
    ...
    $toolbar = array(
        ...
        'save_setting' => '<li class="midNav_last_child"><a href="javascript:void(0)" onclick="save_setting()"><span class="save">Save</span></a></li>',
    );
    $listToolbar = '';
...
//    $mail->AddAddress($fromEmail, $fromName);(bỏ mail info@aodaihousing.com ra khỏi contact 20190702)
...
function vcp_printf($string = "", $lang='ja', $replace = array(), $tags = false){
    $return = "";
...
function helper_get_module_current() {
	$CI =& get_instance();
    $lang = $CI->uri->segment(1);
    
    if ($lang == 'vn' || $lang == 'jp') {
    	$result = $CI->uri->segment(2);
    } else {
    	$result = $CI->uri->segment(1);
    }
    
    return $result;
}

function helper_get_action_current() {
	$CI =& get_instance();
    $lang = $CI->uri->segment(1);
    
    if ($lang == 'vn' || $lang == 'jp') {
    	$result = $CI->uri->segment(3);
    } else {
    	$result = $CI->uri->segment(2);
    }
...
function helper_get_Id_current() {
	$CI =& get_instance();
    $lang = $CI->uri->segment(1);
    
    if ($lang == 'vn' || $lang == 'jp') {
    	$result = $CI->uri->segment(4);
    } else {
    	$result = $CI->uri->segment(3);
    }
...
function helper_get_after_id_current() {
    $CI =& get_instance();
    $lang = $CI->uri->segment(1);

    if ($lang == 'vn' || $lang == 'jp') {
        $result = $CI->uri->segment(5);
    } else {
        $result = $CI->uri->segment(4);
    }
...
function helper_get_page_current() {
	$result = '';
	$allow_page_current = array('page', 'building');
	if (in_array(helper_get_module_current(), $allow_page_current)) {
		$result = helper_get_action_current();
	}

    return $result;
}

function helper_get_sort_current() {
    $CI =& get_instance();
    $lang = $CI->uri->segment(1);
    $result = '';

    if ($lang == 'vn' || $lang == 'jp') {
        $sort = $CI->uri->segment(6);
    } else {
        $sort = $CI->uri->segment(5);
    }

    if ($sort == 'sort') {
        if ($lang == 'vn' || $lang == 'jp') {
            $result = $CI->uri->segment(7);
        } else {
            $result = $CI->uri->segment(6);
        }
    }
...
function helper_special_character($str) {
	return htmlspecialchars($str, ENT_QUOTES, 'UTF-8');
}

function check_rent_min($rent){
    $rent_min = $rent - 100;
    if($rent_min < 0){
        $rent_min = 0;
    }
...
function check_rent_max($rent){
    $rent_max = $rent + 100;
    return $rent_max;
}

/**
 * Check list page is allow responsive
...
function is_responsive() {
    ...
    if ($helper_module == 'building') {
        $allow_responsive = true;
    }

    // Building page: /building_buy_sell/...
    if ($helper_module == 'building_buy_sell' || $helper_module == 'ホーチミンの不動産売買') {
        $allow_responsive = true;
    }

    $page_other_not_reponese = array(

    );

    // Map page: page/住居エリア検索
    // if ($helper_module == 'page' && $helper_action == '住居エリア検索') {
    if ($helper_module == 'page' && !in_array($helper_action, $page_other_not_reponese)) {
        $allow_responsive = true;
    }
    if ($helper_module == 'search' && $helper_action == 'list_data') {
        $allow_responsive = true;
    }
    if ($helper_module == 'houses') {
        $array_responsive_in_houses = array(
            'detail',
            // new arrival
            '最新賃貸物件',
            'new_arrival',
            ...
            // all
            'house_detail_by_condition_all',
            '注目物件',
        );
        if (in_array($helper_action, $array_responsive_in_houses)) {
            $allow_responsive = true;
        }
    }
    if ($helper_module == 'offices') {
        $array_responsive_in_offices = array(
            'detail',
        );
        if (in_array($helper_action, $array_responsive_in_offices)) {
            $allow_responsive = true;
        }
    }
    if ($helper_module == '希望物件リクエスト') {
        $allow_responsive = true;
    }
    if ($helper_module == 'news') {
        $allow_responsive = true;
    }
    if ($helper_module == 'お客様の声'){
    	$allow_responsive = true;
    }
    if ($helper_module == 'contact_request_email'){
    	$allow_responsive = true;
    }
    if ($helper_module == 'tag') {
        $allow_responsive = true;
    }
    return $allow_responsive;
}
 ...
 * Use for debug display and exit data
 */
if ( ! function_exists('fn_convert_label_module')) {
	function fn_convert_label_module($module) {
		if (empty($module)) {
			return $module;
		}
