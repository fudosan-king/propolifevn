...
class BannerHome extends MX_Controller {

	private $module = 'bannerhome';
    private $module_name = 'Banner home';
	private $table = 'banner_home';
	function __construct(){
		parent::__construct();
		$this->load->model('bannerhome_model','model');
				...
				if($_FILES){
					foreach($fileName as $k=>$v){
						if(isset($_FILES['fileAdmincp']['error'][$k])){
							...
							$this->optimizeImage($upload_path. $fileName[$k], 80);*/
						}
					}
				}
				//End Upload Image
	...
	public function get_banner() {
	   $this->load->model('bannerhome/bannerhome_model', 'bannerHome');
	   $item = $this->bannerHome->getBanner();
       $data['item'] = $item;
