...
class BannerTop extends MX_Controller {

	private $module = 'bannertop';
    private $module_name = 'Banner top';
	private $table = 'banner_top';
	function __construct(){
		parent::__construct();
		$this->load->model('bannertop_model','model');
				...
				if($_FILES){
					foreach($fileName as $k=>$v){
						if(isset($_FILES['fileAdmincp']['error'][$k])){
							...
							$upload_path_thumb = BASEFOLDER.'static/uploads/bannerhome/thumb_';
							if(move_uploaded_file($tmp_name[$k], $upload_path . $fileName[$k])){
								$this->optimizeImage($upload_path . $fileName[$k], 1349);
								$this->createThumbs($upload_path . $fileName[$k], $upload_path_thumb . $fileName[$k], 600);
							}
						}
					}
				}
	...
	public function optimizeImage( $pathToImages, $thumbWidth) 
    {
        ...
        foreach($fnames as $fname){
            ...
            if( isset($info['extension']) AND strtolower($info['extension']) == 'jpg')
            {
                ...
                imagejpeg( $tmp_img, "{$pathToImages}" , 50);
            }
        }
    }
	...
	public function get_banner() {
	   $this->load->model('bannertop/bannertop_model', 'bannerTop');
	   $item = $this->bannerTop->getBanner();
       $data['items'] = $item;
