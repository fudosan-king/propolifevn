...
class BannerTopOffice extends MX_Controller {

	private $module = 'bannertopoffice';
    private $module_name = 'Banner top Office';
	private $table = 'banner_top_old';
	function __construct(){
		parent::__construct();
		$this->load->model('bannertopoffice_model','model');
				...
				if($_FILES){
					foreach($fileName as $k=>$v){
						if(isset($_FILES['fileAdmincp']['error'][$k])){
							...
							/*$this->resize_image($upload_path. $fileName[$k]);
							$this->optimizeImage($upload_path. $fileName[$k], 50);*/
						}
					}
				}
				//End Upload Image
    ...
    public function optimizeImage($source, $quality)
    {
	...
	public function get_banner() {
	   $this->load->model('bannertopoffice/bannertopoffice_model', 'bannerTopoffice');
	   $item = $this->bannerTopoffice->getBanner();
       $data['items'] = $item;
