...
<div class="content_table">
	<table cellspacing="0" cellpadding="0" border="0" width="100%">
		<tr>
			...
			<th class="th_left" onclick="sort('title')"><div id="title" class="sort icon_no_sort">Title JP</div></th>
			<th class="th_left" onclick="sort('title')"><div id="title" class="sort icon_no_sort">Title VN</div></th>
			<th class="th_left" onclick="sort('status')"><div id="status" class="sort icon_no_sort">Status</div></th>
			<th class="th_left" width="100" onclick="sort('created')"><div id="created" class="sort icon_sort_asc">Created</div></th>
				...
				<tr class="item_row<?=$i?> <?php ($k%2==0) ? print 'row1' : print 'row2' ?>">
					...
					<td class="th_left td_action">
						<div class="td_relative">
							...
							<?php echo vcp_printf($v->title, 'jp');?>
						</div>
					</td>
					<td class="td_center"><?php echo vcp_printf($v->title, 'vn');?></td>
					<td class="td_center" id="loadStatusID_<?=$v->id?>">
						<a href="javascript:void(0)" onclick="updateStatus(<?=$v->id?>,<?=$v->status?>,'<?=$module?>')">
							<img alt="Checked item" src="<?=PATH_URL.'static/images/admin/icons/'?><?php ($v->status==0) ? print 'uncheck_16x16.png' : print 'check_16x16.png' ?>" />
						</a>
					</td>
					<td class="td_center"><?=date('d-m-Y',strtotime($v->created))?></td>
					<td class="th_last td_center">
						<?php
							if ($v->modified) {
								echo date('d-m-Y',strtotime($v->modified));
							}
						?>
					</td>
				</tr>
