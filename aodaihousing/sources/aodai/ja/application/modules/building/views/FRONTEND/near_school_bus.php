					...
					<div class="product-item1 active">
						<?php if($list): ?>
							<?php foreach($list as $item): ?>
								<?php $link = create_url("building/detail-near-school-bus/" . $item->id . '/' . vcp_printf($item->name, 'jp', $replace)); ?>
								<div class="product-detail-area">
									...
									<div class="product-detail-map">
										<a href="<?php echo $link; ?>">
											<?php //echo vcp_printf($item->name, 'vn'); ?><br>
...
<script>
	...
	$(document).ready(function($) {
		...
		$('html, body').animate({
            scrollTop: $("#scroll-buidling").offset().top - 100
        }, 500);
        /*var $width = $(window).width();
        if($width > 1024){
            $('html, body').animate({
                scrollTop: $("#scroll-buidling").offset().top - 30;
            }, 500);
        }else{
            $('.content-search-block').removeAttr('id');
        }*/
	});
</script>