...
<div class="content_table">
	<table cellspacing="0" cellpadding="0" border="0" width="100%">
		<tr>
            ...
            <th class="th_left" onclick="sort('image')"><div id="title" class="sort icon_no_sort">Image</div></th>
            <th width="70" onclick="sort('status')"><div id="status" class="sort icon_no_sort">Status</div></th>
            <th width="70" onclick="sort('order')"><div id="order" class="sort icon_no_sort">Order</div></th>
					...
					<tr class="item_row<?=$i?> <?php ($k%2==0) ? print 'row1' : print 'row2' ?>">
						...
						<td class="th_left td_action">
							<div class="td_relative">
			                    <div class="action"><a href="<?=PATH_URL.'admincp/'.$module.'/update/'.$v->id?>"><img class="action_first" alt="Edit item" src="<?=PATH_URL.'static/images/admin/icons/edit.png'?>" /></a><img onclick="deleteItem(<?=$v->id?>)" alt="Delete item" src="<?=PATH_URL.'static/images/admin/icons/delete.png'?>" /></div><?=vcp_printf($v->title, 'jp'); ?>
			                </div>
			            </td>
			            <td class="th_left td_action">
			            	<?php if (!empty($v->image)): ?>
			            		<img src="<?php echo PATH_URL;?>/static/uploads/building_buy_sell/<?php echo $v->image;?>" alt="" width="150" />
			            	<?php endif; ?>
			            </td>
