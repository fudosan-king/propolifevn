    ...
    public function search(){
        if($this->device != 'pc'){
            ...
            if(isset($catid) && $catid != ''){
                redirect(PATH_URL.$lang.'houses/house_detail_by_condition_all?catid='.$catid.'&catname='.$catname);
            }else{
                $opt                                  = $this->input->get('opt');
                $cboAreaName                          = $this->input->get('areas');
                $cboLayOut                            = $this->input->get('layout');
                $cboType                              = $this->input->get('type');
                $cboSize                              = $this->input->get('size');
                $cboRentSelect                        = $this->input->get('rent');
                $cboRentSelect                        = $this->input->get('month_rent');
                $cboSizeRentSelect                    = $this->input->get('size_rent');
                $inputMin = '0';
                $inputMax = '6000';
                if(isset($cboRentSelect)  && $cboRentSelect != ''){
                ...
                redirect(PATH_URL.$lang.'search/list_data?opt='.$opt.'&lblMin='.$inputMin.'&lblMax='.$inputMax.'&cboAreaName='.$cboAreaName.'&cboLayOut='.$cboLayOut.'&cboType='.$cboType.'&cboSize='.$cboSize.'&cboRentSelect='.$cboRentSelect.'&cboSizeRentSelect='.$cboSizeRentSelect);
            }
        }
    }

    public function sort($order){
$data['info']=$this->common_model->getSearch($order);
