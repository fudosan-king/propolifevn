    ...
    <div class="row_text_field_first">
		<table cellspacing="0" cellpadding="0" border="0" width="100%">
			<tr>
				...
				<td class="left_text_field">お得な物件:</td>
				<td class="right_text_field"><input <?php if(isset($result->promotion)){if($result->promotion==1){ ?>checked="checked"<?php }}else{ ?>checked="checked"<?php } ?> type="checkbox" class="custom_chk" name="promotion" /></td>
			</tr>
		</table>
	</div>
    ...
    <div class="row_text_field">
		<table cellspacing="0" cellpadding="0" border="0" width="100%">
			<tr>
				...
				<td class=" right_text_condo right_text_field <?php if(isset($result->type) && ($result->type == 1 || $result->type == 2)){echo 'hidden';} ?>">
                    <?php
                        $data_building_type = $building_serviced;
                    ?>
                ...
                <td class=" right_text_serviced right_text_field <?php if(!isset($result->type) || $result->type != '1'){echo 'hidden';}?>">
                    <?php
                    $data_building_type = $building_condo;
                    ?>
                    <select id="e9" style="width:300px" name="building_type1">
                        <option value="">None</option>
                        <?php foreach($data_building_type as $key => $item): ?>
                            <?php
                            $selected = '';
                ...
                <td class=" right_text_all right_text_field <?php if( !isset($result->type) || $result->type != '2'){echo 'hidden';}?>">
                    <?php
                    $data_building_type = $building_all;
                    ?>
                    <select id="e10" style="width:300px" name="building_type2">
                        <option value="">None</option>
                        <?php foreach($data_building_type as $key => $item): ?>
                            <?php
                            $selected = '';
...
<!-- End tags -->  
    ...
    <?php
    $data_db = array();
        if(isset($equipments_db) && count($equipments_db) > 0){
    ...
    <div class="row_text_field">
		<table cellspacing="0" cellpadding="0" border="0" width="100%">
            <tr>
                <td class="left_text_field <?php if(isset($equipments_db) && count($equipments_db) > 0){ echo 'equipment_houses';}?>">Equipments:</td>
				<td class="right_text_field">
    ...
    <?php
        $data_db = array();
        if(isset($facilitie_db) && count($facilitie_db) > 0){
    ...
    <div class="row_text_field">
		<table cellspacing="0" cellpadding="0" border="0" width="100%">
            <tr>
                <td class="left_text_field <?php if(isset($facilitie_db) && count($facilitie_db) > 0){ echo 'facility_houses';}?>">Common facilities:</td>
				<td class="right_text_field">
	...
	<div class="row_text_field">
		<table cellspacing="0" cellpadding="0" border="0" width="100%">
			<tr>
				...
				<td class="right_text_field"><input type="file" name="fileAdmincp[images_thumb]" />
                    <?php if(isset($result->images_thumb)){ if($result->images_thumb!=''){ ?> -
                    <a class="fancyboxClick" href="<?=PATH_URL.'static/uploads/houses/'.$result->images_thumb?>">Review</a><?php }} ?></td>
			</tr>
		</table>
	</div>      
    <div class="row_text_field">
...
<script type="text/javascript">
    ...
    $("#e4").change(function (){
        var value = $(this).val();
        var id    = $('#FormId').val();
        $('.right_text_condo').removeClass('hidden');
        $('.right_text_serviced').removeClass('hidden');
        $('.right_text_all').removeClass('hidden');
        if(value === '0'){
            $('.right_text_serviced').addClass('hidden');
            $('.right_text_all').addClass('hidden');
        }
        if(value === '1'){

            $('.right_text_condo').addClass('hidden');
            $('.right_text_all').addClass('hidden');
        }
        if(value === '2'){
            $('.right_text_condo').addClass('hidden');
