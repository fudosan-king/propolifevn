...
class List_Houses extends MX_Controller {

    private $module = 'list_houses';
    ...
    function __construct() {
        ...
        $this->load->model('list_houses_model', 'model');
...
//        getToolbar($this->module, $toolBar);
    ...
    public function detail() {
        ...
        if (is_numeric($id) == false) {
            ...
            $size = ($item->size) ? vcp_printf($item->size, current_lang_()) . ' m&#178;' : '';
            $rent = ($item->rent) ? vcp_printf($item->rent, current_lang_()) . ' USD' : '';
            ...
            $thumb = !($item->images_thumb) ? $this->config->item('image') : PATH_URL . 'static/uploads/houses/' . $item->images_thumb;
            $seoGoogle = array(
