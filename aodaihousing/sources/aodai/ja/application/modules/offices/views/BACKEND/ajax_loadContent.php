	...
	<table cellspacing="0" cellpadding="0" border="0" width="100%">
		...
		<tr class="item_row<?=$i?> <?php ($k%2==0) ? print 'row1' : print 'row2' ?>">
            ...
            <td class="th_left td_action">
                <a href="<?=PATH_URL.'admincp/'.$module.'/update/'.$v->id?>"><?php echo vcp_printf($v->name_jp, 'jp')?></a>
            </td>
            <?php
            if($v->images_thumb == '') {
                foreach ($images_gallery as $key=>$img) {
                    if ($img->office_id == $v->id) {
                        $images = $img->name_image;
                    }
                }
            }
            ?>
			<td class="td_center" <?php if(($v->images_thumb == '') && ($images == '')){ echo 'style="cursor:default"';}?>>
                <a href="<?=PATH_URL.'admincp/'.$module.'/update/'.$v->id?>">
                    <?php if(($v->images_thumb != '') || ($images != '')){?>
                        <img width="150" src="<?=PATH_URL?>static/uploads/offices/<?php if($v->images_thumb != ''){echo $v->images_thumb;}else{if($images != ''){echo 'gallery/'.$images;}} ?>" />
                    <?php } ?>
                </a>
            </td>
			...
			<td class="td_center">
                <img class="delete" onclick="deleteItem(<?=$v->id?>)" alt="Delete item" src="<?=PATH_URL.'static/images/admin/icons/icon_delete.png'?>" />
            </td>
            <?php
            $date_current = $v->update == ''? $v->created:$v->update;
            ?>
			<td class="th_last td_center"><?=date('Y-m-d',strtotime($date_current))?></td>
		</tr>
		<?php $i++;}}else{ ?>
		<tr class="row1">
