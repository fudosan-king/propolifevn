...
<div id="body">
	<div id="main" style="width: 500px; text-align: left;">
		<div style="float: left; width: 100%; border-bottom: 2px solid #D2147F; padding-bottom: 3px;"><a href="<?=PATH_URL?>"><img width="150" alt="Logo 2dep" src="<?=PATH_URL.'static/uploads/news/medium/default.png'?>" /></a></div>
		<div id="btPrint" style="float: left; width: 100%; margin-top:10px;"><a onclick="printPage();" href="javascript:void(0)"><img alt="Button Print - 2dep" src="<?=PATH_URL.'static/images/bt_print.gif'?>" /></a></div>
			...
			<div class="bg_tag" style="width: 440px;">
				<?php
					...
					foreach($arr_tag as $v){
						if($i==1){
							print '<a style="color: #838486;" href="'.create_url().'tag?t='.trim($v).'">'.$v.'</a>';
						}else{
							print ', <a style="color: #838486;" href="'.create_url().'tag?t='.trim($v).'">'.$v.'</a>';
						}
						$i++;
				...
				<ul>
					<li class="content_promotions" style="width: 480px;">
						<ul>
							...
							<li class="other_news_middle" style="margin-top: 5px;"><a href="<?=create_url().SEOHTML($name_catsub[0]->name).'/'.SEOHTML($name_sub[0]->name).'/'.$v->id.'/'.SEO($v->title)?>"><?=$v->title?></a> (<?=date('d/m',strtotime($v->created))?>)</li>
							<?php } ?>
						</ul>
					</li>
				</ul>
