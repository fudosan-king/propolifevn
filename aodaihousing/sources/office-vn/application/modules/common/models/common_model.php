...
//                $this->db->where(PREFIX.'areas.id', $data['localtion']);
    ...
    public function getCategoryDetailHouse($catId) {
        $sql = "
                ...
                WHERE cli_houses.`status` = 1 AND cli_category.id = $catId AND cli_category.`status` = 1 ORDER BY cli_houses.update desc, cli_houses.created desc LIMIT 0, 6
            ";
        $query = $this->db->query($sql);
    ...
    function getImagesGalleryHouses(){
    ...
    function getImagesGalleryOffices(){
