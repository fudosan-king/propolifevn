...
$is_page_design_interior = false;
if (get_current_controller() == 'page' && get_current_method() == 'page_design_interior') {
    // page/オフィス店舗デザイン・設計施工の流れ
    // page/ベトナム ホーチミン 内装工事 オフィス店舗デザイン・設計施工
    $is_page_design_interior = true;
}

$is_page_interior_furniture = false;
if (get_current_controller() == 'page' && get_current_method() == 'page_interior_furniture') {
    // page/ベトナム ホーチミン オフィス店舗 内装工事
    // page/オフィス-店舗探し-内装工事 クロニクルインテリア
    // page/html_parallax2
    $is_page_interior_furniture = true;
}
...
<head>
    ...
    <link rel="stylesheet" href="<?=PATH_URL.'static/css/'?>/style.css?ver=3.8.2" />
    
    <?php if(current_lang_() == 'vn'): ?>
    <link rel="stylesheet" href="<?=PATH_URL.'static/css/'?>/vn.css?ver=3.8.2" />
    <?php endif ?>
    ...
    elseif(preg_match('/Win/',$agent)){
        echo "<link rel='stylesheet' href='".PATH_URL."static/css/ie.css?ver=3.8.2' type='text/css' media='all' />";
    } 
    elseif(preg_match('/Mac/',$agent)){
        echo "<link rel='stylesheet' href='".PATH_URL."static/css/mac.css?ver=3.8.2' type='text/css' media='all' />";
        if(current_lang_() == 'vn'){
            echo "<link rel='stylesheet' href='".PATH_URL."static/css/vn-mac.css?ver=3.8.2' type='text/css' media='all' />";    
        }
        ?>
        <style type="text/css">
            .menu-item li a span{
    ...
    <link href="<?=PATH_URL?>static/plugin/fancybox/source/jquery.fancybox.css?ver=2.1.5" rel="stylesheet"/>
    <script type="text/javascript" defer="defer" src="<?=PATH_URL?>static/plugin/select2/select2.js"></script>
	<script type="text/javascript" src="<?=PATH_URL?>static/plugin/fancybox/source/jquery.fancybox.pack.js?ver=2.1.5"></script>
	<script type="text/javascript" src="<?=PATH_URL?>static/plugin/jcarousellite/jcarousellite_1.0.1.js?ver=2.1.5"></script>
    <link rel="stylesheet" href="<?=PATH_URL?>static/plugin/jcarousellite/slide.css" type="text/css" media="screen" />
    <script type="text/javascript" defer="defer" src="<?=PATH_URL.'static/js/'?>main.js?ver=2.1.5"></script>
...
<body <?php if($is_page_design_interior == true || $is_page_interior_furniture == true){ ?> class="parallax_body" <?php } ?> >
        ...
        <div class="header">
            ...
            <div class="sologan sologan1">      
               ...
               <a href="<?=PATH_URL?>page/法人設立・起業サポート">
                <img width="196" src="<?=PATH_URL_IMAGES?>static/images/lotus_logo_fix-02.png" />
               </a>
            ...
            <div class="sologan sologan2">
                ...
                <a href="<?=PATH_URL?>page/オフィス店舗デザイン・設計施工の流れ">
                    <img src="<?=PATH_URL_IMAGES?>/static/images/logo-chronicle.png" />
               </a>
                
            </div>

            <?php /*if (mobile_template() == 'pc'): ?>
                <div class='btn btn-danger btn-lg btn-animate btn-redirect-new-domain'>
                    <?php
                        echo change_language('新サイトに移行しました、こちらから新サイトに移動してください', 'Đã move qua site mới, từ đây hay chuyển qua site mới');
                    ?>
                </div>
            <?php endif;*/ ?>
...
<!--                    <strong class="phone">--><?php //echo lang("phone8") ?><!--</strong><br />-->
                </div>
            </div>
    ...
    <!-- Top slide -->
    <?php if($is_page_design_interior == false && $is_page_interior_furniture == false) { ?>
        ...
        <div class="main-content">
            ...
            ?>
                <?php echo modules::run('common/content_search', $catid, $module); ?>
                 ...
                 <h1 class="h1-im">アオザイハウジングでは、ホーチミンの賃貸不動産を仲介手数料無料にてご紹介しております。日本人常駐で、ご案内、契約書類作成、入居後フォロー、<br/>更新手続きまで安心です。お気軽にお問い合わせ下さい。</h1>
                 <?php endif; ?>
            <?php endif; ?>
            <?php if($module == 'home'): ?>
    					...
    					<div class="right-new-item text-intro">
    						<ul>
    							...
    							<li>
    								...
    								<a href="<?=PATH_URL; ?>page/ローカルサービスアパートメント・アパートメントのご紹介">
    								<span>アオザイハウジングではホーチミン市のサービスアパートほぼ全てをご紹介しております。サイゴンビューレジデンス,カンタビル・ホアン・カウ,サマセットビスタ,サマーセット・ホーチミン, サマーセット・チャンセラーコート,ノーフォークマンション,ランカスター,インターコンチネンタル, セドナスイーツ,シティビュー,ランドマーク,アバロン,サイゴンパビロン,シャーウッドレジデンス, サイゴンコート,クレッセントレジデンス,ウォーターフロント,カプリ,オークウッドなど気になるサービスアパートが御座いましたらお気軽にご相談下さい。</span> <br />  
    								</a>
    							</li>

                                <li>
                                    <a href="<?=PATH_URL; ?>page/ローカルサービスアパートメント・アパートメントのご紹介">
                                    <span>アオザイハウジングではホーチミン市のコンドミニアムほぼ全てをご紹介しております。サイゴンパール、マナー、シティガーデン、ビンホームズセントラルパーク、ホライズン、セーリングタワー、シーリバービュー、アセント、マステリタオディエン、カンタビルプレミア、サマセットビスタ、エステラ、タオディエンパール、パノラマ、ガーデンコート、リバーパーク、サンライズシティ、リッチレーンなど気になるコンドミニアムが御座いましたらお気軽にご相談下さい。</span> <br />  
                                    </a>
                                </li>
    						</ul>
    					</div>
...
<?php if ($is_page_design_interior == true): ?>
    <?php $this->load->view('FRONTEND/parallax'); ?>
<?php elseif ($is_page_interior_furniture == true): ?>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $('.btn-redirect-new-domain').on('click', function(){
            // Use function get_url_redirect() in file: application/helpers/common_helper.php
            window.location.href = '<?php echo get_url_redirect();?>';
        });
    });
</script>
</body>
