...
function wpcf7_text_shortcode_handler( $tag ) {
    ...
    } else if ( $tag->name == 'your-email' ) {
        $icon = '<span class="input-group-addon"><i class="fa fa-envelope"></i></span>';
    } else if ( $tag->name == 'your-tel' ) {
        $icon = '<span class="input-group-addon"><i class="fa fa-phone"></i></span>';
    } else {
        $icon = '<span class="input-group-addon"><i class="fa fa-file-text-o"></i></span>';
    }

	$atts = wpcf7_format_atts( $atts );

	$html = sprintf(
		'<div class="wpcf7-form-control-wrap %1$s"><div class="input-group">' . $icon . '<input class="form-control" %2$s /></div></div>%3$s',
