...
if ( ! function_exists( 'woocommerce_template_loop_product_thumbnail_customer' ) ) {
     ...
     * @subpackage  Loop
     */
    function woocommerce_template_loop_product_thumbnail_customer() {
        echo woocommerce_get_product_thumbnail_customer();
    }
}
...
if ( ! function_exists( 'woocommerce_template_loop_price_customer' ) ) {
     ...
     * @subpackage  Loop
     */
    function woocommerce_template_loop_price_customer() {
        wc_get_template( 'loop/price_customer.php' );
    }
}
...
if ( ! function_exists( 'woocommerce_get_product_thumbnail_customer' ) ) {
	...
	function woocommerce_get_product_thumbnail_customer( $size = array(360, 360), $deprecated1 = 0, $deprecated2 = 0 ) {
		global $post;
		echo '<span class="lazy-container lazyloaded"><span class="lazy_preloader"></span>';
		if ( has_post_thumbnail() ) {
			return get_the_post_thumbnail( $post->ID, $size );
		} elseif ( wc_placeholder_img_src() ) {
			return wc_placeholder_img( $size );
...
if ( ! function_exists( 'woocommerce_cart_totals' ) ) {
	...
	function woocommerce_cart_totals() {
		wc_get_template( 'cart/cart-totals-customer.php' );
	}
}
...
if ( ! function_exists( 'woocommerce_button_proceed_to_checkout' ) ) {
	...
	function woocommerce_button_proceed_to_checkout() {
		wc_get_template( 'cart/proceed-to-checkout-button-customer.php' );
	}
}
...
if ( ! function_exists( 'woocommerce_breadcrumb_customer' ) ) {
    ...
    function woocommerce_breadcrumb_customer( $args = array() ) {
        ...
        wc_get_template( 'global/breadcrumb_customer.php', $args );
    }
}
...
if ( ! function_exists( 'woocommerce_form_field_customer' ) ) {
	 ...
	 * @todo This function needs to be broken up in smaller pieces
	 */
	function woocommerce_form_field_customer( $key, $args, $value = null ) {
		$defaults = array(
			'type'              => 'text',
			'label'             => '',
			'description'       => '',
		...
		$field = '';
		$label_id = $args['id'];
		if ($key == 'billing_address_2' || $key == 'order_comments' ) {
			$field_container = '<div class="col-xs-12"><div class="form-group"><p class="form-row %1$s" id="%2$s">%3$s</p></div></div>';
		} else {
			$field_container = '<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12"><div class="form-group"><p class="form-row %1$s" id="%2$s">%3$s</p></div></div>';
		...
		switch ( $args['type'] ) {
			...
			case 'select' :
				...
				if ( ! empty( $args['options'] ) ) {
					...
					$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="form-control select '.esc_attr( implode( ' ', $args['input_class'] ) ) .'" ' . implode( ' ', $custom_attributes ) . ' placeholder="' . esc_attr( $args['placeholder'] ) . '">
							' . $options . '
						</select>';
				}
...
if ( ! function_exists( 'woocommerce_add_header' ) ) {
...
if ( ! function_exists( 'woocommerce_add_footer' ) ) {
    function woocommerce_add_footer() {
        echo '
            </div>
        ';
    }
...
if ( ! function_exists( 'woocommerce_add_header_product' ) ) {
    function woocommerce_add_header_product() {
        echo '
            <div class="row">
            <div class="col-lg-12">
...
if ( ! function_exists( 'woocommerce_add_footer_product' ) ) {
    function woocommerce_add_footer_product() {
        echo '
            </ul>
            </div>
            </div>
...
if ( ! function_exists( 'woocommerce_add_header_product_cat' ) ) {
    function woocommerce_add_header_product_cat() {
        echo '
            <div class="bg-default shop">
        ';
    }
}

if ( ! function_exists( 'woocommerce_add_footer_product_cat' ) ) {
    function woocommerce_add_footer_product_cat() {
        echo '
            </div>
        ';
    }
...
if ( ! function_exists( 'woocommerce_show_product_images_customer' ) ) {
    function woocommerce_show_product_images_customer() {
        ...
        if ( has_post_thumbnail() ) {
            ...
            $image          = get_the_post_thumbnail( $post->ID, array( 684, 479), array(
                'title' => $image_title,
                'alt'   => $image_title
                ) );
            $image = str_replace("width=" ,"", $image);
            ...
            echo apply_filters( 'woocommerce_single_product_image_html', sprintf( '<a data-href="%s" itemprop="image" class="woocommerce-main-image zoom" title="%s" data-rel="prettyPhoto' . $gallery . '">%s</a>', $image_link, $image_caption, $image ), $post->ID );
...
if ( ! function_exists( 'woocommerce_product_thumbnails_customer' ) ) {
    function woocommerce_product_thumbnails_customer() {
        do_action( 'woocommerce_product_thumbnails' );
    }
}
...
if ( ! function_exists( 'woocommerce_single_product_summary_content' ) ) {
    function woocommerce_single_product_summary_content() {
        ...
        if ( $heading ):
          echo '<span style="display:block;font-weight:bold;text-decoration:underline">' . $heading . '</span>';
        endif;
        the_content();

        // Image
        echo '
            <div id="sliderProduction" style="position: relative; top: 0px; left: 0px; width:425px; height:70px; overflow: hidden;margin-top:10px;padding-bottom:20px">
            ...
            <div u="slides" style="cursor: move; position: absolute; left: 0px; top: 0px; width:425px; height:70px; overflow: hidden;">
        ';
        if ( $attachment_ids ) {
            $loop       = 0;
            $columns    = apply_filters( 'woocommerce_product_thumbnails_columns', 3 );
                ...
                foreach ( $attachment_ids as $attachment_id ) {
                    ...
                    echo apply_filters( 'woocommerce_single_product_image_thumbnail_html', sprintf( '<div align="center"><a href="%s" class="%s fancybox-buttons" data-fancybox-group="button" title="%s" data-rel="prettyPhoto[product-gallery]">%s</a></div>', $image_link, $image_class, $image_caption, $image ), $attachment_id, $post->ID, $image_class );
