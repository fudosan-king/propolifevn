...
function receivemail(){
	global $post;
	$email=$_REQUEST['mail'];
	$arg = array('post_type' => 'receive-e-mail','orderby' => 'date','order' => 'desc','posts_per_page' =>-1,'status' => array('publish','private'));
	$the_query = new WP_Query($arg);
	while ( $the_query->have_posts() ) : $the_query->the_post();
	if($email==$post->post_title){
		$result = alertDialog(20);
		echo $result;
		exit();
	}	
...
add_action('wp_ajax_receivemail','receivemail');
add_action('wp_ajax_nopriv_receivemail','receivemail');
/*-----------------------------------------------------------------------*/
