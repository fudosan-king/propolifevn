...
if($_POST['entrygo0']){
	//最初のPW
	if($_POST['e_infopw']){
		$e_infopw = htmlspecialchars($_POST['e_infopw'], ENT_QUOTES);
		if($e_infopw == "starmica"){
		}
		else{
			$e_infopw_msg = "パスワードが違います。";
			$errorflag = 1;
		}
	}
	else{
		$e_infopw_msg = "パスワードのご入力をお願いいたします。";
		$errorflag = 1;
	}
	if($errorflag == 1){
...
//アンケート   確認画面　　送信
//----------------------------------------------------------------
if($_POST['entrygo2'] OR $_POST['entrygo3'] OR $_POST['entrysousin']){
	//----------------------------------------------------------------------
	//購入のきっかけ
	//----------------------------------------------------------------------
	//もっと広い部屋に住みたい
...
?>
						...
						<input type="submit" name="entrysousin" value="送信する"  class="sousin_button">
						...
						<input type="hidden" name="e_infopw" value="<?=$e_infopw?>">
						<input type="hidden" name="e_infopw" value="<?=$e_infopw?>">
						</div>
						</FORM>
					</section>
...
?>
						...
						<td>
						<input type="text" name="e_tel" style="width:50%;ime-mode:disabled" value="<?=$e_tel?>" placeholder="例：03-5776-2688" class="form-control">
						<span class="supportmsg">半角英数（例：03-5776-2688）</span>
<?php
	if($e_tel_msg){
?>
			...
			<div class="row">
				<div id="content" class="col third-fourth right">
					<section id="sec01">
						<div class="midashi-s"><span>アンケート</span></div>
						<p class="bottom-10">
						スター・マイカより郵送された書面記載のパスワードをご入力してください。
						</p>
						<div class="clear-box"></div>
						...
						<tr>
						<th>
						パスワード<span class="icon-required">必須</span>
						</th>
						<td>
						<input type="text" name="e_infopw" style="width:70%;ime-mode:disabled" value="<?=$e_infopw?>" placeholder="パスワードをご入力ください。" class="form-control">
<?php
	if($e_infopw_msg){
?>
					<div id="errmsg"><?=$e_infopw_msg?></div>
<?php
	}
?>
						</td>
						...
						<div class="botancenter">
						<input type="submit" name="entrygo0" value="次に進む"  class="sousin_button">
						</div>
						</FORM>
					</section>
				</div>
